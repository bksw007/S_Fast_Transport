...
export default function ContactPicker({ actor, value, onChange }: { actor: UserProfile; value: StopContact; onChange: (value: StopContact) => void }) {
  ...
  return <div className="contact-picker">
    <label className="contact-picker-select"><span>เลือกผู้ติดต่อ</span><select value={value.id} disabled={loading || Boolean(error)} onChange={e => { const selected = items.find(item => item.id === e.target.value); onChange(selected ? { id: selected.id, name: selected.name, phone: formatPhoneNumber(selected.phone), notes: "" } : { id: "", name: "", phone: "", notes: "" }); }}><option value="">{loading ? "กำลังโหลด…" : "ระบุเอง หรือเลือกจากสมุดรายชื่อ"}</option>{value.id && !options.some(item => item.id === value.id) && <option value={value.id}>{value.name} (ข้อมูลที่เลือกไว้)</option>}{options.map(item => <option key={item.id} value={item.id}>{item.name} · {formatPhoneNumber(item.phone)}</option>)}</select></label>
    {error && <small role="alert">โหลดสมุดรายชื่อไม่สำเร็จ สามารถระบุผู้ติดต่อเองได้</small>}
    <div className="contact-phone-preview">{formatPhoneNumber(value.phone) || "เบอร์โทรจะแสดงเมื่อเลือกผู้ติดต่อ"}</div>
    ...
    <label>เบอร์โทรผู้ติดต่อ<input type="tel" inputMode="numeric" value={formatPhoneNumber(value.phone)} maxLength={12} required={Boolean(value.name.trim())} pattern="[0-9]{3}-[0-9]{3}-[0-9]{4}" onChange={e => onChange({ ...value, phone: formatPhoneNumber(e.target.value) })} /></label>
